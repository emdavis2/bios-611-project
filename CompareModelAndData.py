...
# PRW_wt_weight = float(PRW_wt_params_readlines[1].split('=')[1])
...
# data_PRWweighted_sim = run_weighted_PRW_sim(Nwalkers, dt, time, PRW_wt_weight, PRW_wt_w_kappa, PRW_wt_theta_kappa)
...
file_lines.append('pval for D/T data and PRWPB={}'.format(pval_PRWPB))
#plt.text(.05, 0.8, 'pvalue {} data and PRW PB={}'.format(region,pval_PRWPB), fontsize = 12, bbox = dict(facecolor = 'red', alpha = 0.1))
...
file_lines.append('pval for D/T data and PRWsim={}'.format(pval_PRW))
#plt.text(.05, 0.7, 'pvalue {} data and PRW={}'.format(region,pval_PRW), fontsize = 12, bbox = dict(facecolor = 'red', alpha = 0.1))
